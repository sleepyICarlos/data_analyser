...
folder = "C:\\Users\hartz\Documents\D3\Areas" #"\\janeway\User AG Bluhm\Hartz\sciebo\promotion\6_LogsDataAnalysis\1_data\IHT_probe_station\2018-03-15_photocurrent_experiments\D3\Areas"
start_name = "15-3-18"
sample_name = "D3"
        ...
        if file.endswith(end_name):
            ...
            for i in np.arange(len(contents)):
                if contents[i] == file_identifier:
                    ...
                    measurement_description.append(contents[i+1])
                    break

            pad_distance.append(pad_distance_value)
            files.append(file)
            new_value=np.genfromtxt(folder+"\\"+file,usecols=0, skip_header=2).astype(float)
...
pad_distance.sort()
voltages = np.concatenate((np.array([0]),np.genfromtxt(folder+"\\"+files[0],usecols=1, skip_header = 2))) 
...
def fit_linear_zoom(x_data, y_data, measurement_index, fit_points, sweep_direction):        #fit_points == number of fitpoints, # sweep_direction== 0, 1 for up and downsweep, respectively
    ...
    y_fit_parameters=np.polyfit(x, y, 3)
